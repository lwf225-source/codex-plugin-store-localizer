...
Windows real-device UI acceptance is **pending**. Its Authenticode validation does not pin a publisher. macOS requires the expected signed identity and full app integrity; the launcher stops if validation fails. The screenshots below demonstrate visible translations, not compatibility with every app build.
...
The GitHub connector descriptions near the bottom of Image 3 remain English. The supplied screenshots do not establish all-string coverage, the capture OS, app version, or Windows acceptance.
...
The [dictionary examples](examples.html) and `translation-examples.png` illustrate bundled translations; they are not app screenshots.
